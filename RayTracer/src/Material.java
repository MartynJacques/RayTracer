...
class Lambertian extends Material {
	...
	public boolean scatter(Ray r_in, HitRecord rec, Vec3 attentuation, Ray scattered) {
		Vec3 target = rec.p.add(rec.normal).add(Vec3.randomInUnitSphere()); // random scatter
...
class Dielectric extends Material {
	...
	public boolean scatter(Ray r_in, HitRecord rec, Vec3 attentuation, Ray scattered) {
		...
		if (Vec3.dot(r_in.direction(), rec.normal) > 0) { // if within hemisphere of normal
			...
			ni_over_nt = ref_idx;
			// cosine = ref_idx*Vec3.dot(r_in.direction(),
			// rec.normal)/r_in.direction().length(); //cosine of angle * ref_idx?
